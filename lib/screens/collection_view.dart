...
final reportDataProvider = FutureProvider.family<Map<String, dynamic>, ReportParams>((
  ...
  if (kIsWeb) {
    ...
    final aggregatorDir = await FileService().getAggregatorPath(
      params.dbKey,
      external: true,
    );
    ...
    final result = await IsolateWorker.instance
        .execute<Map<String, dynamic>>('runReportGenerationPipeline', {
          'dbName': params.dbKey,
...
class _CollectionViewState extends ConsumerState<CollectionView>
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging) {
        ...
        ref
            .read(dbSearchProvider.notifier)
            .reset(); // Reset search state upon tab switches!
  ...
  Future<void> _handleBatchDelete() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          "Are you sure you want to permanently delete ${_selectedKeys.length} records? This action cannot be undone.",
        ),
        actions: [
          TextButton(
  ...
  Future<void> _handleDone(ElementDb db) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "This will export the database locally, backup to Google Drive, and generate your Daily and Monthly reports. Continue?",
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
    ...
    if (confirmed == true) {
      ...
      ); // Yield to allow browser to complete first paint of dialog
      ...
      try {
        ...
        final aggregatorContent = widget.contents.firstWhere(
          (c) => c.type == ContentType.aggregator,
        );
        ...
        if (mounted) {
          ...
          final aggIdx = widget.contents.indexWhere(
            (c) => c.type == ContentType.aggregator,
          );
          if (aggIdx != -1) {
          ...
          FeedbackToast.success(
            context,
            "Database finalized and reports generated!",
  ...
  Future<void> _importDb(ElementDb db) async {
    try {
      ...
      if (result != null) {
        ...
        if (data != null && data is List) {
          ...
          try {
            ...
            if (mounted) {
              Navigator.pop(context); // Close loading dialog
              FeedbackToast.error(context, "Import Error: $e");
            }
          }
        } else {
      ...
      if (mounted) {
        FeedbackToast.error(context, "Import Failed: $e");
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    if (widget.contents.isEmpty) {
      return const Scaffold(
        body: Center(child: Text("No contents in this Schema")),
      );
    }

    final searchState = ref.watch(dbSearchProvider);
    final currentContent = widget.contents[_currentTabIndex];
    final bool isDatabaseLanding =
        currentContent.type == ContentType.database &&
        searchState.showLandingPage;
    final bool isKeyboardVisible = MediaQuery.of(context).viewInsets.bottom > 0;
    final bool isSearchActive =
        searchState.showLandingPage || searchState.isSearching;
    ...
    if (currentContent.type == ContentType.database) {
      final db = currentContent.service as ElementDb;
      if (db.dbHeader.isNotEmpty &&
          db.dbHeader[0] is List &&
          db.dbHeader[0].isNotEmpty) {
        title = db.dbHeader[0][0].toString();
      }
      if (db.dbHeader.length > 1 &&
          db.dbHeader[1] is List &&
          db.dbHeader[1].isNotEmpty) {
      ...
      if (agg.reports.isNotEmpty) {
        ...
        if (firstReport.header.isNotEmpty &&
            firstReport.header[0] is List &&
            firstReport.header[0].isNotEmpty) {
          title = firstReport.header[0][0].toString();
        }
        if (firstReport.header.length > 1 &&
            firstReport.header[1] is List &&
            firstReport.header[1].isNotEmpty) {
    ...
    return PopScope(
      ...
      child: Scaffold(
                ...
                title: searchState.isSearching
                    ? Row(
                        children: [
                          ...
                          GestureDetector(
                            onTap: () =>
                                setState(() => _isExactMatch = !_isExactMatch),
                            child: Container(
                                ...
                                if (currentContent.type ==
                                  ...
                                  IconButton(
                                    icon: SvgPicture.string(
                                      '<svg viewBox="0 0 24 24"><path fill="currentColor" d="M14,12L10,8V11H2V13H10V16M20,18V6C20,4.89 19.1,4 18,4H6A2,2 0 0,0 4,6V9H6V6H18V18H6V15H4V18A2,2 0 0,0 6,20H18A2,2 0 0,0 20,18Z" /></svg>',
                                      width: 26,
                                      height: 26,
                                    ...
                                    onPressed: () {
                                      final db =
                                          currentContent.service as ElementDb;
                                      _importDb(db);
                                    },
                                    constraints: const BoxConstraints(),
                                    padding: const EdgeInsets.symmetric(
                                  ...
                                  IconButton(
                                    icon: SvgPicture.string(
                                      '<svg viewBox="0 0 24 24"><path fill="currentColor" d="M23,12L19,8V11H10V13H19V16M1,18V6C1,4.89 1.9,4 3,4H15A2,2 0 0,1 17,6V9H15V6H3V18H15V15H17V18A2,2 0 0,1 15,20H3A2,2 0 0,1 1,18Z" /></svg>',
                                      width: 26,
                                      height: 26,
                                      colorFilter: const ColorFilter.mode(
                                        Colors.brown,
        ...
        floatingActionButton: hideCradleFab
            ? null
            : FloatingActionButton(
                ...
                onPressed: () {
                  if (_currentTabIndex == 0) {
                    ...
                    final currentShowLanding = ref
                        .read(dbSearchProvider)
                        .showLandingPage;
                    ref
                        .read(dbSearchProvider.notifier)
                        .setShowLandingPage(!currentShowLanding);
                  } else {
...
class _DatabaseViewState extends ConsumerState<_DatabaseView>
    with AutomaticKeepAliveClientMixin<_DatabaseView> {
  @override
  ...
  bool _isSearchViewActive = false;
  ...
  void initState() {
    super.initState();
    _landingSearchKey = GlobalKey(
      debugLabel: 'landingSearchField_${widget.db.key}',
    );
    _landingFocusNode = FocusNode();
    _landingFocusNode.addListener(() {
      if (_landingFocusNode.hasFocus) {
        _isSearchViewActive = true;
      }
      if (mounted) setState(() {});
    });
  ...
  Future<void> _triggerSearch(String query) async {
    ...
    final bool showLanding =
        ref.read(dbSearchProvider).showLandingPage &&
        widget.searchQuery.isEmpty;
    final String activeFilter = showLanding ? 'Active' : _currentFilter;

    final results = await widget.db.searchAsync(
      trimmed,
      exact: widget.isExactMatch,
      filter: activeFilter,
    );
    if (mounted) {
      setState(() {
  ...
  Future<void> _init({bool forced = false}) async {
    ...
    _activeBusinessKeyName = await SqliteHelper.getBusinessUniqueKey(
      widget.schemaTitle,
    );
    ...
    if (mounted) {
      setState(() {
        ...
        if (_selectedElementForDetail != null) {
          final matched = widget.db.elements.where(
            (e) => e.key == _selectedElementForDetail!.key,
          );
  ...
  Widget _buildSearchLandingPage() {
    ...
    // Adaptive logo sizing: 18% of the smaller dimension, clamped between 80px and 180px
    final double logoSize =
        (screenWidth < screenHeight ? screenWidth : screenHeight) * 0.18;
    final double clampedLogoSize = logoSize.clamp(80.0, 180.0);

    final settings = ref.watch(settingsProvider);
    final googleUser = ref.watch(googleUserProvider);

    Widget logoWidget;
    if (settings.useGoogleProfilePic &&
        googleUser != null &&
    ...
    } else if (settings.customSearchPicPath != null) {
      logoWidget = ClipRRect(
        borderRadius: BorderRadius.circular(16),
        child: Image.file(
          File(settings.customSearchPicPath!),
          width: clampedLogoSize,
          height: clampedLogoSize,
          fit: BoxFit.cover,
          errorBuilder: (context, error, stackTrace) => SvgPicture.asset(
    ...
    final bool showTopStuckLayout = showResults || _isSearchViewActive;

    // 2. Search Input pill row (Common)
    Widget searchBar = Container(
      key: _landingSearchKey,
      constraints: BoxConstraints(
        maxWidth: showTopStuckLayout ? double.infinity : 580.0,
      ),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Row(
        children: [
          if (showTopStuckLayout) ...[
            IconButton(
              icon: const Icon(Icons.arrow_back, color: Color(0xFF6B1524)),
            ...
            child: Container(
              ...
              child: Text(
                ...
                style: TextStyle(
                  ...
                  fontWeight: FontWeight.bold,
                  decoration: TextDecoration.underline,
                  decorationColor: widget.isExactMatch
                      ? Colors.white
                      : Colors.black54,
                  decorationThickness: 2.0,
        ...
        children: [
          ...matchingRecords.map((element) {
            if (element.components.isEmpty ||
                element.components.first is! ListHeader) {
            ...
            final metaData =
                (recordData.values.first as Map)['__meta__']?['time'] ?? {};
            ...
            final titleWidgets = header.displayHeader(
              context,
              headerType: 'title',
              allComponents: element.components,
            );
            final elementWidgets = header.displayHeader(
              context,
              headerType: 'elements',
              allComponents: element.components,
              onChanged: () async {
                            ...
                            child: DefaultTextStyle(
                              ...
                              child: Column(
                                ...
                                children: titleWidgets
                                    .map(
                                      (group) =>
                                          Wrap(spacing: 20, children: group),
                                    )
                                    .toList(),
                              ),
                            ),
                      ...
                      _buildGroupedSection(
                        ...
                        DefaultTextStyle(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (elementWidgets.length > 1)
                                Wrap(spacing: 20, children: elementWidgets[1]),
              ...
              child: _isDbEmpty
                  ? Container(
                      ...
                      decoration: BoxDecoration(
                        color: const Color(
                          0xFF6B1524,
                        ).withOpacity(0.04), // Subtle Velvet Crimson tint
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: const Color(0xFF6B1524).withOpacity(0.15),
                          width: 1.0,
                        ),
                      ),
                      child: Column(
    ...
    final double topSpacerHeight = showTopStuckLayout ? 8.0 : verticalSpacing;
    final double middleSpacerHeight = showTopStuckLayout ? 0.0 : verticalSpacing;
    final double logoContainerHeight = showTopStuckLayout ? 0.0 : clampedLogoSize;
    final double bottomSpacerHeight = showTopStuckLayout ? 8.0 : 0.0;
    final double bottomOffsetHeight = showTopStuckLayout ? 0.0 : bottomOffset;
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            AnimatedOpacity(
              opacity: showTopStuckLayout ? 0.0 : 1.0,
              duration: const Duration(milliseconds: 300),
              curve: Curves.easeInOut,
            ...
            Flexible(
              ...
              child: AnimatedOpacity(
                opacity: showTopStuckLayout ? 1.0 : 0.0,
                duration: const Duration(milliseconds: 300),
                curve: Curves.easeInOut,
                child: Visibility(
                  visible: showTopStuckLayout,
                  child: SingleChildScrollView(
                    child: Column(
                      children: [resultsList, const SizedBox(height: 80.0)],
                    ),
                  ),
                ),
              ),
    ...
    return GestureDetector(
      ...
      onTap: () {
        _landingFocusNode.unfocus();
        FocusScope.of(context).unfocus();
        if (_landingSearchController.text.isEmpty) {
          setState(() {
            _isSearchViewActive = false;
          });
        }
    ...
    if (draftData.isNotEmpty) {
      ...
      if (fields is Map) {
        ...
        if (_activeBusinessKeyName != null) {
          final val = _findValueRecursively(
            Map<String, dynamic>.from(fields),
            _activeBusinessKeyName!,
          );
          if (val != null && val.isNotEmpty) {
  ...
  void _discardDraft(ElementModel draft) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          "Are you sure you want to discard this draft ($displayName)? All edits will be permanently lost.",
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  void onAdd() async {
    ...
    newElement.key =
        "Draft ${widget.db.elements.length + _drafts.length + 1} ($timeStr)";
    ...
    final saved = await Navigator.push<bool>(
      ...
      MaterialPageRoute(
        builder: (context) =>
            ElementEditor(db: widget.db, element: newElement, isNew: true),
      ),
    );
  ...
  void _openEditor(ElementModel element) async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ElementView(db: widget.db, element: element),
      ),
    );
    if (!mounted) return;
    _init(forced: true);
  ...
  Future<void> _handleCardAction(String action, ElementModel element) async {
    if (action == 'archive') {
      ...
      final confirmed = await _showConfirm(
        "PERMANENT DELETE",
        "Are you sure? This action CANNOT be undone and the data will be lost forever.",
        isDestructive: true,
      );
      if (confirmed) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool showLanding =
        searchState.showLandingPage && widget.searchQuery.isEmpty;
    if (showLanding) {
      ...
      return Scaffold(
        ...
        resizeToAvoidBottomInset:
            true, // Allows search bar to slide up and stay visible above the keyboard
        body: _buildSearchLandingPage(),
        floatingActionButton: (widget.selectedKeys.isEmpty && !hideFab)
            ? _buildSpeedDialFab()
            : null,
      );
    }
    ...
    if (useSplitView) {
      return Scaffold(
          ...
          children: [
            Row(
              children: [
                ...
                SizedBox(
                  width: (MediaQuery.of(context).size.width * 0.35).clamp(
                    320.0,
                    480.0,
                  ),
                  child: Column(
                    children: [
                      if (!searchState.isSearching)
                        Container(
                          color: Colors.white,
                          padding: const EdgeInsets.only(top: 2.7, bottom: 3.0),
                          child: SingleChildScrollView(
                        ...
                        child: filteredElements.isEmpty
                            ...
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final isSelectedInSplit =
                                      _selectedElementForDetail?.key ==
                                      element.key;
                                  final isSelectedForBatch = widget.selectedKeys
                                  ...
                                  final borderHighlightColor =
                                      ...
                                      : (isSelectedInSplit
                                            ? const Color(0xFFE9967A)
                                            : Colors.grey.shade200);
                                  final borderHighlightWidth =
                                      (isSelectedForBatch || isSelectedInSplit)
                                      ? 2.0
                                      : 1.0;
                                  final cardBgColor = isSelectedInSplit
                                      ? const Color(
                                          0xFFE9967A,
                                        ).withOpacity(0.05)
                                      : Colors.white;
                                  ...
                                  if (element.components.isNotEmpty &&
                                      element.components.first is ListHeader) {
                                    final header =
                                        element.components.first as ListHeader;
                                    header.setDbSchema(widget.db.dbSchema);

                                    final Map<String, dynamic> recordData =
                                        element.fetch();
                                    final metaData =
                                        (recordData.values.first
                                            as Map)['__meta__']?['time'] ??
                                        {};
                                    final isArchived = metaData.containsKey(
                                      'a',
                                    );
                                    final isDeleted = metaData.containsKey('d');
                ...
                Expanded(
                  child: _selectedElementForDetail == null
                      ...
                      : ElementView(
                          ...
                          key: ValueKey(
                            "split_detail_${_selectedElementForDetail!.key}",
                          ),
                          onChanged: () => setState(() {}),
                          onBack: () {
                            setState(() {
        ...
        floatingActionButton: widget.selectedKeys.isEmpty
            ? _buildSpeedDialFab()
            : null,
      );
    }
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (element.components.isNotEmpty &&
                            ...
                            final Map<String, dynamic> recordData = element
                                .fetch();
                            final metaData =
                                (recordData.values.first
                                    as Map)['__meta__']?['time'] ??
                            ...
                            final isSelected = widget.selectedKeys.contains(
                              element.key,
                            );
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              if (elementWidgets.length > 1)
                                                Wrap(
                                                  spacing: 20,
                                                  children: elementWidgets[1],
                                                ),
                                              if (elementWidgets.length >
                                                  2) ...[
                                                const Divider(
                                                  height: 24,
                                                  color: Colors.black12,
  ...
  Widget _buildSpeedDialFab() {
    ...
    final double dockHeight = 64.0;
    final double dockMargin = bottomSafeArea > 0 ? 4.0 : 8.0;
    final double bottomOffset = dockHeight + dockMargin;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w900,
                  color: headingColor ?? Colors.blueGrey,
                  letterSpacing: 1.8,
                ),
              ),
              const Spacer(),
          ...
          const Divider(height: 12, thickness: 0.8, color: Colors.black12),
          content,
        ],
      ),
    );
...
class _ElementViewState extends State<ElementView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.element.components.isNotEmpty &&
        widget.element.components.first is ListHeader) {
      final header = widget.element.components.first as ListHeader;
      final titleWidgets = header.displayHeader(
        context,
        headerType: 'title',
        allComponents: widget.element.components,
      );
      if (titleWidgets.isNotEmpty) {
        titleWidget = Column(
          ...
          mainAxisSize: MainAxisSize.min,
          children: titleWidgets
              .map((group) => Wrap(spacing: 20, children: group))
              .toList(),
        );
      }
    }
          ...
          IconButton(
            ...
            onPressed: () async {
              await Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      ElementEditor(db: widget.db, element: widget.element),
                ),
              );
              setState(() {});
              ...
              builder: (context, setModalState) => cClone.editor(
                ...
                cbNotifyParent: (notifier, data, observers) {
                  if (cClone is SimpleAccount) {
                    setModalState(() {
                      cClone.updateObservers(
                        notifier,
                        data,
                        observers,
                        cClone.getComponentAtIndex(0) ?? cClone,
                      );
                    });
...
class AggregatorReportView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AggregatorReportView> createState() =>
      _AggregatorReportViewState();
}
...
class _AggregatorReportViewState extends ConsumerState<AggregatorReportView> {
  ...
  void _share(String filePath) async {
    try {
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    await Share.shareXFiles([
                      XFile(filePath),
                    ], subject: 'AnyDb Report: ${widget.report.key}');
                  } catch (e) {
                    messenger.showSnackBar(
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  try {
                    ...
                    FeedbackToast.error(context, "Upload failed: $e");
                  }
                },
              ),
              const SizedBox(height: 16),
      ...
      if (mounted) {
        FeedbackToast.error(context, "Share Error: $e");
      }
    }
  }
  ...
  Widget _buildTable(List<dynamic> aoa) {
    ...
    final filteredDataRows = dataRows.where((row) {
      ...
      return row.any(
        (cell) => _formatValue(cell).toLowerCase().contains(query),
      );
    }).toList();
  ...
  String _formatValue(dynamic c) {
    ...
    if (c is int && c > 1000000000000) {
      return DateFormat(
        'dd/MM/yy',
      ).format(DateTime.fromMillisecondsSinceEpoch(c));
    }
    if (c is num) {
  ...
  Widget build(BuildContext context) {
    ...
    return reportAsync.when(
      ...
      data: (reportData) {
        ...
        final List<dynamic> aoa = reportData['aoa'] as List? ?? [];
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              if (widget.selectedRange != null)
                                Text(
                                  "Range: ${DateFormat.yMd().format(widget.selectedRange!.start)} - ${DateFormat.yMd().format(widget.selectedRange!.end)}",
                                  style: const TextStyle(fontSize: 12),
                                ),
                            ],
                          ),
                        ...
                        children: [
                          ListTile(
                            ...
                            onTap: () async {
                              ...
                              if (picked != null)
                                widget.onDateChanged(picked, null);
                            },
                          ),
                        ],
                      ),
              ...
              Expanded(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: const Color(
                      0xFFFF8A80,
                    ), // Opaque deepOrangeAccent.shade100
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: Colors.black12),
                  ),
                  child: Padding(
