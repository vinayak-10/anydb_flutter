...
abstract class Extractor {
  ...
  String predicatedName(Map<String, dynamic> pred, String data) {
    ...
    final formatPreds = predicates
        .where((p) => p['operation'] == pred['operation'])
        .toList();
    final p = formatPreds.firstWhere(
      (p) => p['name'] != pred['name'],
      orElse: () => pred,
    );
  ...
  Future<Map<String, dynamic>> applyPredicate(
...
class ExtractorDatabase extends Extractor {
  ...
  Future<void> populateWithData(List<Map<String, dynamic>> elements) async {
    ...
    final allElements = _segregate(
      elements,
      types: ["Active", "Archived", "Deleted"],
    );
    logger.log(
      "ExtractorDatabase Isolate: Segregated ${allElements.length} Total elements.",
    );
    ...
    logger.log(
      "ExtractorDatabase Isolate: Flattened into ${_data.length} total rows.",
    );
    ...
    for (var p in predicates) {
      if (p is Map) {
        logger.log(
          "ExtractorDatabase Isolate: Applying sequential predicate: ${p['operation']} (${p['name']})",
        );
  ...
  Future<void> populateRecords() async {
    ...
    logger.log(
      "ExtractorDatabase: Fetched ${elements.length} raw elements from storage.",
    );
    ...
    logger.log(
      "ExtractorDatabase: Segregated ${allElements.length} Total elements (Active/Archived/Deleted).",
    );
    ...
    logger.log(
      "ExtractorDatabase: Flattened into ${_data.length} total rows from ${allElements.length} records.",
    );
    ...
    if (workingRows.isNotEmpty && workingRows.length < _rows.length) {
      logger.log(
        "ExtractorDatabase: Refined rows to ${workingRows.length} (had date value).",
      );
      _rows.clear();
      _rows.addAll(workingRows);
    }
    ...
    for (var p in predicates) {
      if (p is Map) {
        logger.log(
          "ExtractorDatabase: Applying sequential predicate: ${p['operation']} (${p['name']})",
        );
        await applyPredicate(Map<String, dynamic>.from(p));
      }
    }
    ...
    logger.log(
      "ExtractorDatabase: Final population complete. Filtered Rows: ${_rows.length}",
    );
  }

  List<Map<String, dynamic>> _segregate(
    ...
    for (var row in rows) {
      ...
      for (var column in cols) {
        final String destKey =
            (column is Map ? (column['title'] ?? column['column']) : column)
                .toString();
        final String srcKey =
            (column is Map ? (column['column'] ?? column['title']) : column)
                .toString();
  ...
  Future<Map<String, dynamic>> applyPredicate(
    ...
    switch (operation) {
      case "date":
        ...
        final DateTime d = data is DateTime
            ? data
        ...
        logger.log(
          "ExtractorDatabase: Date filter ($searchKey) matched ${predicated.length} records for $formattedDate.",
        );
      ...
      case "sort":
        ...
        if (colKey.isNotEmpty) {
          _rows.sort((a, b) {
            final valA =
                CellHelper.unwrap(
                  a[colKey] ?? _findValueInsensitive(a, colKey),
                )?.toString() ??
                "";
            final valB =
                CellHelper.unwrap(
                  b[colKey] ?? _findValueInsensitive(b, colKey),
                )?.toString() ??
                "";
...
class ExtractorReport extends Extractor {
  ...
  Future<Map<String, dynamic>> applyPredicate(
    ...
    if (pred['operation'] == 'date') {
      if (data == null) return {};
      final DateTime date = data is DateTime
          ? data
          : (DateTime.tryParse(data.toString()) ?? DateTime.now());
  ...
  Future<void> _prepare(
    ...
    logger.log(
      "ExtractorReport: Searching for source sheets '$sourceReportName' in file '$fileName' (timestamp: $timestamp)",
    );

    final sheetNames = await _workbookService.getSheetNames(
      fileMeta,
      sourceReportName,
    );
    logger.log(
      "ExtractorReport: Found ${sheetNames.length} source sheets: $sheetNames",
    );
      ...
      DateTime? parseSheetDate(String name) {
        ...
        if (parts.length >= 3) {
          ...
          if (month == null) {
            ...
            month =
                months[monthPart.toLowerCase().substring(
                  0,
                  3.clamp(0, monthPart.length),
                )];
          }
    ...
    for (var sheetName in sheetNames) {
      ...
      if (sheetData.isNotEmpty) {
        logger.log(
          "ExtractorReport: Sheet '$sheetName' has ${sheetData.length} rows.",
        );
            ...
            if (match != null) {
              ...
              if (rowNum != null) {
                ...
                if (rowIdx < sheetData.length &&
                    colIdx < sheetData[rowIdx].length) {
                  final val = CellHelper.unwrap(sheetData[rowIdx][colIdx]);
                  row[title] = val;
                  logger.log(
                    "ExtractorReport: Sheet '$sheetName' ($colAlpha$rowNum) -> $title = $val",
                  );
                } else {
                  logger.log(
                    "ExtractorReport: Coordinate $colAlpha$rowNum out of bounds for sheet '$sheetName'",
                  );
                  row[title] = 0;
                }
              } else {
              ...
              for (int i = 0; i < headers.length; i++) {
                final h = CellHelper.unwrap(
                  headers[i],
                ).toString().trim().toLowerCase();
                if (h == title.toLowerCase() ||
                    h == "total $title".toLowerCase()) {
                  foundIdx = i;
                  break;
                }
              }
...
class ExtractorService {
  ...
  Future<Map<String, dynamic>> generate(
    ...
    return await _intf.extractor!.applyPredicate(
      pred,
      data: data,
      getFileName: _intf.pIntf['getFileName'],
    );
  }
}
