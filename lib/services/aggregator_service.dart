...
class AggregatorReport {
    ...
    for (var row in rowList) {
      ...
      ext.init(Map<String, dynamic>.from(row), {
        'generate': (pd, {DateTime? timestamp}) =>
            pIntf['generate'](pd, timestamp: timestamp),
        'getFileName': (j, {DateTime? timestamp}) =>
            pIntf['getFileName'](j, timestamp: timestamp),
      });
      extractor.add(ext);
    }
  }
  ...
  Map<String, dynamic> applyMeta(Map<String, dynamic> meta) {
    ...
    if (dateVal is DateTime) {
      ...
      formattedName = extractor[0].predicatedName(
        nmeta['predicate'] ?? {},
        nmeta['entry'] ?? '',
      );
  ...
  Map<String, dynamic> generateData(Map<String, dynamic> j) {
    ...
    logger.log(
      "AggregatorReport: Generating data for $key. Records: ${rawRecords.length}",
    );
    ...
    summary.forEach((title, formula) {
      final val = FormulaEngine.evaluate(
        formula.toString(),
        records,
        dataHeaders,
        titleToKeyMap,
      ...
      logger.log(
        "AggregatorReport: Summary '$title' = $val (Formula: $formula)",
      );
    });
    ...
    return {
      ...
      "source":
          j['extra']?['source'] ??
          (extractor.isNotEmpty ? extractor[0].extractor?.source : {}),
...
class AggregatorService {
  ...
  Future<Map<String, dynamic>> generate(
    ...
    final entryName = report.extractor[0].predicatedName(
    ...
    logger.log(
      "AggregatorService: Target Sheet Name: $finalSheetName (Original: $sheetName)",
    );

    logger.log(
      "AggregatorService: Fetching data from source (force: $force)...",
    );
  ...
  Future<String> generateWorkbook(
    AggregatorReport report, {
    dynamic date,
    DateTime? timestamp,
    bool force = false,
  }) async {
    final reportData = await generate(
      report,
      date: date,
      timestamp: timestamp,
      force: force,
  ...
  Future<String> generateMonthlyBatch(
    DateTime monthDate, {
    bool force = false,
  }) async {
    try {
      ...
      if (generatedDays > 0) {
        ...
        lastPath = result['path'];
        logger.log(
          "AggregatorService: Monthly batch complete. Path: $lastPath",
        );
        if (!kIsWeb) {
          workbook.clearCache();
        }
  ...
  Map<String, dynamic> getFileName(
    ...
    return {"aggregator": key, "collection": collection, "fileName": fileName};
  }
